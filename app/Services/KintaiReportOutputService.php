...
class KintaiReportOutputService
{
    public function getPulldownBase()
    {
        // ロールによって拠点の選択肢を変動
        ...
        if(Auth::user()->role_id >= 31){
            $bases = Base::where('base_id', Auth::user()->base_id)->get();
        } 
        return $bases;
    }
    ...
    public function getOutputFileName($month, $base_name)
    {
        // 出力年月をフォーマット
