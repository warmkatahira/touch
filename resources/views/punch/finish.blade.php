...
<x-app-layout>
    ...
    <div class="py-5 mx-5">
        <div class="grid grid-cols-12">
            ...
            <p class="col-start-4 col-span-6 text-center text-4xl bg-emerald-100 border-b-4 border-emerald-400 rounded-t-lg py-2 h-3/4">退勤画面</p>
            <p class="col-start-1 col-span-12 text-4xl py-3 text-center text-white bg-blue-500 rounded-t-lg">退勤画面</p>
            <!-- 時計を表示 -->
        ...
        <form method="get" id="punch_begin_confirm_form" action="{{ route('punch_finish.input') }}" class="m-0">
            ...
            <div class="grid grid-cols-12 gap-4 mt-10">
                @foreach($employees as $employee)
                    <button type="submit" name="employee_no" class="col-span-3 bg-black text-white text-center text-2xl rounded-lg px-8 py-10 hover:bg-sky-500" value="{{ $employee->employee_no }}">{{ $employee->employee_name }}</button>
                @endforeach
            </div>
        </form>
    </div>
    <!-- モーダル -->
    <div id="punch_finish_confirm_modal" class="fixed hidden z-50 inset-0 bg-gray-900 bg-opacity-60 overflow-y-auto h-full w-full">
        <div class="relative mx-auto shadow-lg rounded-md bg-white">
            ...
            <div class="flex justify-between items-center bg-black text-white text-xl rounded-t-md px-4 py-2">
                <h4>出勤打刻画面</h4>
                <button class="punch_finish_confirm_modal_close"><i class="las la-window-close"></i></button>
            </div>
            <!-- モーダルボディ -->
            <div class="p-10">
                <form method="post" id="punch_finish_confirm_form" action="{{ route('punch_begin.confirm') }}" class="m-0">
                    @csrf
                    <!-- 日付と時間 -->
            ...
            <div class="px-4 py-2 border-t border-t-gray-500 grid grid-cols-1">
                <a class="punch_finish_confirm_modal_close cursor-pointer rounded-lg text-white bg-red-400 hover:bg-sky-500 text-center p-4 col-span-1">
                    キャンセル
                </a>
            </div>
        </div>
