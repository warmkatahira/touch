<x-app-layout>
    <div class="py-5 mx-5">
        <div class="grid grid-cols-12 gap-4">
            ...
            <a href="{{ route('kintai_modify.index', ['kintai_id' => $kintai->kintai_id]) }}" class="col-start-12 col-span-1 text-xl py-4 rounded-lg text-center bg-pink-200 mb-5">修正</a>
        </div>
        <div class="grid grid-cols-12">
            ...
            <p class="col-span-1 border border-black text-sm py-2 text-center mt-1">{{ $kintai->early_work_enabled == 0 ? '通常' : '早出' }}</p>
            ...
            <p class="col-span-1 border border-black text-sm py-2 text-center mt-1">{{ number_format($kintai->out_return_time / 60, 2) }}</p>
            <p class="col-start-1 col-span-1 bg-black text-white text-center py-2 text-sm mt-1">休憩取得時間</p>
            <p class="col-span-1 border border-black text-sm py-2 text-center mt-1">{{ number_format($kintai->rest_time / 60, 2) }}</p>
            <p class="col-span-1 bg-black text-white text-center py-2 text-sm mt-1">稼働時間</p>
            <p class="col-span-1 border border-black text-sm py-2 text-center mt-1">{{ number_format($kintai->working_time / 60, 2) }}</p>
            <p class="col-span-1 bg-black text-white text-center py-2 text-sm mt-1">残業時間</p>
            <p class="col-span-1 border border-black text-sm py-2 text-center mt-1">{{ number_format($kintai->over_time /60, 2) }}</p>
        </div>
